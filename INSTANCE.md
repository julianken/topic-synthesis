...
## Status
**Status: walking skeleton COMPLETE; GCP deploy underway.** The full local e2e ships — a topic typed in the app produces a browsable, interactive curriculum (multi-agent pipeline → Postgres → sandboxed artifact iframes), and every run is observable via the published `@eleatic/eval` trace seam. The GCP **foundation is provisioned** via Terraform (`infra/`): a dedicated `topic-synthesis-prod` project with Cloud SQL Postgres 16, Artifact Registry, Secret Manager, and the runtime SA — see `docs/decisions/0001-deployment-orchestration-and-swappability.md`. The durable `GcpEngine` (Postgres step-memoization) + the Cloud Run Service/Job land in the remaining deploy PRs per `docs/plans/walking-skeleton.md`. Build/test commands: `AGENTS.md` → "Working in the tree".
