...
**Stack:** TypeScript + Next.js (App Router, React 19) front end; Postgres (the app's system-of-record — also holds workflow `step_results`) + Redis (vestigial: it backed the now-dropped Trigger.dev; see [`docs/decisions/0001`](docs/decisions/0001-deployment-orchestration-and-swappability.md)) via Docker; Node ≥ 20. Package manager is **npm**.
...
- `npm run lint:boundaries` — dependency-cruiser import fence: the business-logic layers (`domain`/`llm`/`pipeline`/`engine`/`store`/`eval`/`trace`) must not import the frontend (`next`/`react`/…), keeping the core a Next-free deployable; a second rule confines `@eleatic/eval` (its `better-sqlite3`/`express` deps) to the single `src/trace/eleatic-adapter.ts`, so it never enters the app bundle. The decoupling guards (ADR 0001 §4), CI-enforced via `.github/workflows/import-fence.yml`.
- `npm test` — Vitest unit tests (`npm run test:watch` to watch).
- `npm run build` — production Next.js build; `npm run dev` — dev server.
- `npm run skeleton -- --topic "<topic>" [--level …] [--depth N] [--audience "…"] [--cheap] [--max-nodes N] [--max-questions N] [--trace [path]]` — run the pipeline end-to-end over the in-process engine and print the curriculum + per-model cost. `--cheap` runs every stage on Haiku, `--max-nodes N` caps synthesized pages, and `--max-questions N` caps the research fan-out (each question drives a web search — the main cost driver) — together they keep a test run to ~pennies. `--dump-html <dir>` writes each synthesized page's HTML to `<dir>/<slug>.html` so you can open a generated page in a browser. `--persist` writes the whole run to Postgres via `persistRun` (needs `DATABASE_URL` + a migrated DB — `docker compose up -d && npm run db:migrate`) and prints the `/curriculum/<id>` route the app serves it at. `--trace [path]` records the run's per-stage cost/token spans to a local `@eleatic/eval` SQLite store (one run + an `_analysis` row + a row per built page, each a span tree); explore with `npx @eleatic/eval serve --db <path>` (no path → an ephemeral `:memory:` store). It shares the run id with `--persist` so the trace links the curriculum. Needs a provider API key in the env (e.g. `ANTHROPIC_API_KEY`); uses no durable engine.

**Layout:** everything lives under `src/` (Next.js App Router via `src/app`). `src/app/` (Next.js routes + `globals.css`, which materializes the `DESIGN.md` §0 tokens; `page.tsx` is the intake form → `POST api/generate/route.ts` (mints a runId, runs the pipeline in-process fire-and-forget on cheap + capped models, `persistRun`s on completion — production would dispatch to the Cloud Run Job instead) → redirects to the hub `curriculum/[id]` (`getCurriculum` → tiered tiles via the pure `curriculum/view.ts`; while a run is in flight it shows a generating state that polls `api/curriculum/[id]/status/route.ts` until the row lands); `curriculum/[id]/[slug]` is the detail page (the built page's artifact in a no-`allow-same-origin` iframe, keyed by URL-safe slug); `artifact/[pageId]/route.ts` serves a stored page's HTML in a strict-CSP sandbox built by the pure `artifact/serve.ts`), `src/domain/` (pure types + identity/settings/stage-contracts — no I/O), `src/llm/` (the only LLM-SDK import site — Vercel AI SDK + provider packages; `registry` resolves a `{provider,model}` to a model, `client` wraps text/structured completions (+ `searchWeb`, Anthropic web-search grounding) with per-call cost, plus model tiers + the `provider:model` pricing map), `src/pipeline/` (stages over the `src/domain` contracts — the ANALYSIS LLM stages `planner` / `researcher` (web-search-grounded; drops any finding citing a non-retrieved source) / `graph`, the SYNTHESIS LLM stages `spec` → `code` (standalone HTML) → `critic` (pass/fail verdict), plus the pure deterministic `coverage-gate` (route built/text/soon by coverage; reject duplicate slugs, drop edges to unknown nodes + break cycles by dropping back-edges, so LLM graph quirks can't crash the run) + `hub` (tiered sitemap assembly, `built` only when routed-built AND critic-passed) + `run-pipeline` (orchestrates plan → researchers fan-out → graph → gate → per-built-node spec/code/critic → hub over an injected `Engine`, threading every per-call cost; accepts per-stage model overrides — the workflow_version arm / cheap-mode — a `maxNodes` synthesis cap, and an injectable `StageBundle` so any single stage is swappable for a test or eval arm) + `ports.ts` (the swap-seam catalog: `StageBundle`/`defaultStages` + the named ports — Engine · StageDeps · StageModel · StoreDeps · TraceSink); stages take an injectable `StageDeps` + a `model` param so they unit-test with no live model), `src/engine/` (the `Engine` step-memoization seam + an in-process `InlineEngine` for tests/dev; the durable Postgres-backed `GcpEngine` — run as a Cloud Run Job, no Trigger.dev — is a later PR), `src/store/` (Postgres schema + access — `repo.ts`: `persistRun` writes a whole run atomically + `getCurriculum`/`getPage` read it back by content identity, over an injectable pool), `src/eval/` (the `run-skeleton` CLI driver — runs the pipeline over the `InlineEngine` and prints the curriculum + cost; the `npm run skeleton` entry), `src/trace/` (the OBSERVABILITY seam behind the `TraceSink` port: `span.ts` collects per-call spans, the pure `reduce.ts` folds them to an eleatic run + rows, `eleatic-adapter.ts` is the SOLE `@eleatic/eval` import site — a devDep fenced out of the app bundle; wired into the CLI via `--trace`. Records carry no wall-clock, so spans omit `durationMs`). The durable `GcpEngine` (Postgres step-memoization, run as a Cloud Run Job) and the GCP deployment land in subsequent PRs. The deployment + orchestration + swappability architecture — GCP serverless (Cloud Run Service + Job), code-orchestration (no managed workflow tool), one Cloud SQL Postgres for app records *and* `step_results`, and **every workflow component swappable behind a port** (engine, stages, LLM client, store, model — the testing + eval-arm seam) — is recorded in [`docs/decisions/0001-deployment-orchestration-and-swappability.md`](docs/decisions/0001-deployment-orchestration-and-swappability.md), which **supersedes** the Engine/Orchestration/deploy sections of `docs/plans/walking-skeleton.md`.
