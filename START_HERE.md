...
- **Unfilled template** — files still contain `{{PLACEHOLDERS}}` (e.g. `{{PROJECT_NAME}}`, `{{OWNER}}/{{REPO}}`). It is a reusable scaffold, not a product yet. To stand up a product, **fill** the placeholders from [`.seed/placeholders.json`](.seed/placeholders.json) (project-bootstrap fill-mode, or `scripts/fill-template.sh`).
...
1. **[`INSTANCE.md`](INSTANCE.md)** — *instance facts*: which product, the local-folder-vs-`gh`-slug identity (`{{LOCAL_FOLDER}}/` ↔ `{{OWNER}}/{{REPO}}`), the optional Figma file + node map, and the optional Mergify/review infra. Go here first for "what repo is this and how does it ship."
