...
Reads are **owner-scoped**: a lesson, its generation status, and its pages are
...
sandboxed artifact route authorizes **through the owning lesson, not the page
id**: a `pageId` is a content hash *shared across curricula* by design, so it is not <!-- concept-drift-ok: the `concept_page` content-atom shared across curricula — RETAINED table concept, ADR-0003 -->
