...
describe('GET /api/lesson/[id]/status — the live timeline + research poll (issue #61 + live-research Stage 1)', () => {
  it('returns the owner-scoped steps AND research AND code during a run, even while the curriculum is not yet persisted (ready false)', async () => {
    getSessionIdentity.mockResolvedValue({ sub: 'owner-1' });
    // Pre-persist window: getLesson is still null (not ready) but the owner sees the timeline + feeds.
  ...
  it('returns [] / null for steps AND research AND code for a NON-owner (no existence oracle) — no reader runs', async () => {
