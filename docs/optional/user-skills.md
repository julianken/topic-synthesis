...
| Layer | Lives in | Owns |
| --- | --- | --- |
| **Repo-local (canonical method + instance facts)** | [`.claude/skills/`](../../.claude/skills/) | The generic five-section PR-body method ([`creating-prs`](../../.claude/skills/creating-prs/SKILL.md)), the bot-agnostic anti-slop rubric ([`reviewing`](../../.claude/skills/reviewing/SKILL.md)), and the {{PROJECT_NAME}} instance facts that plug into them ([`pr-workflow`](../../.claude/skills/pr-workflow/SKILL.md), plus `issue-authoring` / `issue-plan-review`). A worktree-isolated subagent reads these directly. |
| **User-level (optional overlay)** | `~/.claude/skills/` (per-developer, shared across repos) | The bot **identity + credential mechanics** (`reviewing-as-{{REVIEW_BOT}}`, optional — only when the review-bot module is enabled), the merge **command flow** (`mergify-merge-workflow`, optional — only when the Mergify module is enabled), the screenshot paste flow (`pr-screenshots-via-user-attachments`), and the **portable** namesake of the method (user-level `creating-prs`). |

The split is deliberate: a cold-start agent that can read only `.claude/skills/` is **not dependent** on a developer's `~/.claude/skills/` to describe the PR body or apply the review rules. The user-level layer adds only what is genuinely personal/cross-repo (credentials, the developer's shared method copy) — it never owns something the repo can't function without. The pieces it adds are also the **optional modules** — the bot overlay is moot when `{{REVIEW_BOT}}` is blank, the Mergify command flow moot when no `.mergify.yml` is present.

## When the overlay applies

- **It loads on top of the repo-local copy, not instead of it.** `reviewing-as-{{REVIEW_BOT}}` *extends* the repo-local `reviewing` rubric with the bot identity and credential loading — it doesn't fork the rubric. You read the repo-local rubric to *judge* a diff; you load the overlay only to *post* the gating verdict as the bot.
- **On conflict:** the repo-local skill wins for anything {{PROJECT_NAME}}-specific (the ruleset, what's in our template) and for the **generic method** itself (`creating-prs` / `reviewing`). The user-level skill wins only for the **portable** form of the method and for the bot identity/credential mechanics the repo-local rubric deliberately doesn't carry.
- **No-drift:** the method exists as two mirrored pairs — repo-local `creating-prs` ↔ user-level `creating-prs`, and repo-local `reviewing` ↔ user-level `reviewing-as-{{REVIEW_BOT}}`. A change to either copy of a pair must update the other in the **same PR**, and the PR Summary must say so. (This rule lives in `AGENTS.md` → "Skill ownership"; it binds whether or not you've installed the user-level layer.)
