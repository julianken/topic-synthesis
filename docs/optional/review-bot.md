...
**Optional.** This module is enabled by setting `julianken-bot` (a blank value disables it). When enabled, this repo gates merge on a fresh approving review from a non-author collaborator, and in practice that reviewer is a dedicated machine-user, `@julianken-bot`. A template consumer can adopt a review bot, use an ordinary human reviewer, or run no review gate at all — the **review content** (the anti-slop rubric) is bot-agnostic and lives in [`.claude/skills/reviewing/SKILL.md`](../../.claude/skills/reviewing/SKILL.md), which carries no credentials.
...
> The dispatch wiring may name `@julianken-bot` as the required poster; if `julianken-bot` is blank (module skipped), an ordinary human reviewer satisfies the gate instead.
...
1. Create a machine-user GitHub account, set `julianken-bot` to its handle, and add it as a collaborator with the permission to submit reviews.
