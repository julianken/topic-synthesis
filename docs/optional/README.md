...
| [`review-bot.md`](./review-bot.md) | `julianken-bot` set (blank disables) | A dedicated machine-user reviewer (`@julianken-bot`) to satisfy a per-HEAD review gate | a human reviewer (or no review gate) is fine for your repo |
| [`figma.md`](./figma.md) | `` set (blank disables) | Reading a Figma design system over the read-only MCP | your design source of truth isn't Figma |
